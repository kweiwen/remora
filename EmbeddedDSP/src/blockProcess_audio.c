...
{
	DigitalDelayLine_NEW(&dl, 44100 * 3, 2);
	dl._time = 8192;

	oscillator_frequency = 440;
	oscillator_gain = 0.75;
	oscillator_model = 0;
	Oscillator_NEW(&osc);
//
//	DigitalDelayLine_DEL(&dl);
//	free_space = heap_space_unused(2);
//	printf("free space: %d\n", free_space);
//
//	DigitalDelayLine_NEW(&dl, 44100 * 3, 2);
//	dl._time = 8192;
...
//	printf("free space: %d\n", free_space);
//
//	PitchShift_NEW(&pitchshifter, 4096);
//	FeedbackDelayNetwork_NEW(&fdn, 2);
}
...
void processSample(float *buffer, unsigned int buffersize)
{
	...
	{
		buffer[i] = osc.Process(&osc, oscillator_frequency, 44100, oscillator_model, 0) * oscillator_gain;
//		buffer[i] = dl.Process(&dl, buffer[i]) * 4;
//		buffer[i] = pitchshifter.Process(&pitchshifter, buffer[i]);
//		buffer[i] = fdn.Process(&fdn, buffer[i]);
	}
}
...
{
	DigitalDelayLine_DEL(&dl);
	PitchShift_DEL(&pitchshifter);
	FeedbackDelayNetwork_DEL(&fdn);
}
